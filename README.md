...
- editable `NSTableView` cells plus a raw-content `fx` formula bar where Return commits without moving the active cell, Tab commits and advances one column while retaining worksheet keyboard ownership, and the next typed character starts editing the newly active cell; Tab wraps from the final column to column A of the next row, while Tab at the final worksheet cell retains that active cell, and Escape restores the original raw content
